...
its norm in `E`. -/
@[simp] lemma add_subgroup.coe_norm {S E : Type*} [semi_normed_group E] [set_like S E]
  [add_subgroup_class S E] {s : S} (x : s) :
  ∥(x : s)∥ = ∥(x:E)∥ :=
rfl
...
-/
@[norm_cast] lemma add_subgroup.norm_coe {S E : Type*} [semi_normed_group E] [set_like S E]
  [add_subgroup_class S E] {s : S} (x : s) :
  ∥(x : E)∥ = ∥(x : s)∥ :=
rfl
